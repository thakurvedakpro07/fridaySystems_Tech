...
const LockIcon = (p) => (
  <svg {...iconProps} {...p}><rect x="3" y="11" width="18" height="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg>
);
...
export default function ConversationFeed({
  ...
  // Internal notes are staff/freelancer-only — a customer can never see this
  // toggle, matching the server's own filtering of is_internal comments out
  // of the customer's feed.
  const canPostInternal = role !== "customer";
  ...
  const handleSend = async (e) => {
    ...
    setSubmitting(true);
    try {
      await addComment(ticketId, draftMessage.trim(), canPostInternal && isInternal);
      onDraftChange("");
      // Reset to public after every send — internal mode shouldn't silently
      // persist into the next message and risk a reply meant for the
      // customer being sent as an internal-only note (or vice versa).
      setIsInternal(false);
      refetch();
      toast(canPostInternal && isInternal ? "Internal note added" : "Message sent", "success");
    } catch {
        ...
        <textarea
          ...
          placeholder={
            canPostInternal && isInternal
              ? "Write an internal note… (visible only to staff and engineers)"
              : "Write a message… (Ctrl+Enter to send, paste an image or drop a file)"
          }
          className={`input-base resize-none py-3 text-[15px] leading-relaxed font-sans placeholder:text-slate-400
            ${canPostInternal && isInternal ? "border-amber-300 focus:ring-amber-300/60 bg-amber-50/40" : ""}`}
        />
        <div className="flex items-center justify-between mt-2">
          <div className="flex items-center gap-1">
            ...
            {canPostInternal && (
              <button
                ...
                title={isInternal ? "Posting as an internal note — click to post publicly instead" : "Post as an internal note (staff/engineers only)"}
                className={`inline-flex items-center gap-1.5 px-2.5 h-8 rounded-lg text-xs font-semibold transition-colors
                  ${isInternal
          ...
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-lg
                       ...
                       ${canPostInternal && isInternal ? "bg-amber-500 text-white" : "bg-brand-gradient text-white"}`}
          >
            {submitting
              ? (canPostInternal && isInternal ? "Adding…" : "Sending…")
              : canPostInternal && isInternal
                ? (<>Add Note <LockIcon className="w-3.5 h-3.5" /></>)
