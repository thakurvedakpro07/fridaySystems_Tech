...
import { previewInvitation, acceptInvitation } from "../api/organizations";
...
const STATUS_MESSAGES = {
  ...
  expired: "This invitation has expired. Ask the organization admin to send a new one.",
};
...
export default function InvitationAcceptPage() {
  ...
  const handleAccept = async (e) => {
    ...
    try {
      ...
      setUser(data.user);
      toast(`Welcome to ${preview.organization_name}!`, "success");
    ...
    return (
      <CardShell>
        <div className="text-center">
          ...
          <h1 className="text-xl font-bold text-slate-900 mb-2">{preview.organization_name}</h1>
    ...
    return (
      <CardShell>
        <div className="mb-6 text-center">
          ...
          <h1 className="text-xl font-bold text-slate-900 mb-1">Join {preview.organization_name}</h1>
          <p className="text-sm text-slate-500">
            {preview.invited_by_email || "A team member"} invited <span className="font-semibold">{preview.email}</span> to join as {preview.role === "org_admin" ? "an Organization Admin" : "a Member"}.
          </p>
        </div>
  ...
  return (
    <CardShell>
      <div className="mb-6 text-center">
        ...
        <h1 className="text-xl font-bold text-slate-900 mb-1">Join {preview.organization_name}</h1>
        <p className="text-sm text-slate-500">
          {preview.invited_by_email || "A team member"} invited <span className="font-semibold">{preview.email}</span> to join as {preview.role === "org_admin" ? "an Organization Admin" : "a Member"}. Create a password to get started.
        </p>
      </div>
